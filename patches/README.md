...
Everything nvkvm changes in **upstream QEMU** is in this directory: four patch
files, 46 added lines and 2 removed, against the `v9.2.0` tag
...
is nothing nvkvm-specific left in it once upstream has the fix.
...
NVIDIA). `0001` and `0002` are downstream by nature: they register a device that
is not upstream and claim a virtio ID that is not assigned.
