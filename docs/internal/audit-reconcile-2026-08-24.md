...
## Read this first: what `main` has closed since `68a35c0`

**This report was written against `68a35c0` and merged forward onto `65aa69f`.** In
between, `main` took `sec-easy-batch` (P-3, P-5, P-7, A-21, U-13) and `stub-window`
(U-9, U-14, `iso_mmap_translate`), plus the DRM `GET_DEV_INFO` ABI fix. Four verdicts
below are therefore **superseded by later merges, not wrong when written**:

| finding | this report says | status at `65aa69f` |
|---|---|---|
| **A-21** | open on `main`, fixed on unmerged `sec-easy-batch` | **FIXED** — `nvkvm_mmap_host.c:961, 990-1008, 1020-1055, 1092, 1161-1177` |
| **P-3** | writable alias fixed, non-PIE half open | **FIXED, both halves** — `src/stub/Makefile:32,38`, `verify-pie` at `:55-61` |
| **P-5** | open, code unchanged | **DECIDED** — uid separation is not required; namespaces are the boundary |
| **P-7** | open, harness only | **GATED** — opt-in behind `NVKVM_DEV_HARNESS_INSECURE_RW`; narrowed, not removed |

Everything else in this report was **re-verified against the code at `65aa69f`** and
holds. Line references throughout were written against `68a35c0` and have drifted —
where a citation was re-checked, the corrected `65aa69f` line is given inline and
marked *"citation corrected at `65aa69f`"*. Four were wrong by more than drift
(A-6, A-15, A-16(3), A-18 — A-18 named the wrong *file*); all four are corrected in
place, and none of them changed a verdict.

**One id-hygiene note, since this report is the only place it is written down.**
`audit-boundaries-2026-08-20.md` and `audit-guest-pointers.md` each have their own
**A-1** and **A-2**, and they are four different findings. The recommendation is
`BD-` / `GP-` / `PR-` prefixes per document, adopted for **new** findings only — the
existing ids are deliberately *not* renumbered, because they appear in commit messages
and in code comments. See the numbering-hazard section at the end of
...
| **A-5** | **fixed** | **PARTIAL — the host-side half was never done. See R-1.** | The doc's fix had two halves. Guest half done (`nvkvm_ioctl.c:143`, justification `:127-142`, sibling gate `:371`) — but that is **guest code, which this audit's own method excludes as a control**. Host half *not* done: the chain in `nvkvm_req_ioctl_on_isolate` (`nvkvm_isolate_handlers.c:2091-3534`) still evaluates the graphics gate `:2451-2458`, then `'d'` `:2461-2474`, then NVKMS `:2476-2522`, and the non-`'F'` default-deny **last** at `:2523-2530`, so a type-`'d'` record still bypasses every `'F'`-keyed gate. No type↔`dev_id` cross-check exists anywhere in that function. **This is the dangerous direction and it is the origin of R-1.** **Found while re-checking at `65aa69f`:** the M-A default-deny *comment* has drifted ~90 lines away from the arm it documents — it sits at `:2432-2440`, immediately before the `'d'` branch, so a reader at `:2432` sees "default-deny any non-`'F'`-type cmd **here**" and then the `'d'` branch, which reinforces exactly the false belief A-5 is about. Move the comment with the code when the reorder lands. |
| A-6 | fixed | **PARTIAL** | `MMAP_ON_ISOLATE` checks session + isolate (`nvkvm_isolate_handlers.c:3615-3616`); `MUNMAP_ON_ISOLATE` now refuses a token the named isolate does not own. **Citation corrected at `65aa69f`:** that refusal lives inside `iso_mmap_free()` itself (`:473-487`, owner test `:479-481`), called from the handler at `:4000` — not at `:3623-3625`. But the code's own comment (`:3979-3994`, inside the S-2 block `:3963-3995`) flags a residual the doc's table does not: `req->isolate_id` is guest-supplied with no session anchor on the wire, so naming a neighbour's isolate *and* its token still passes. Same residual as P-4. |
...
| A-15 | table: fixed / §6: open | **CONFIRMED FIXED — §6 prose was stale, corrected** | Filter built at `nvkvm_stub.c:3105-3205`; `__NR_clone3` / `__NR_clone` / `__NR_fork` / `__NR_vfork` appear only in comments (`:3182`, `:3499`), never in an `ALLOW_IF` entry. F6-1 tombstone `:3182-3192`. **Citation corrected at `65aa69f`:** the "no longer claims to block fork" comment is *not* above `apply_seccomp()` (whose own header comment is `:3084-3088`) — it is the F6-1 block in `main()` at `:3496-3503`, just before the `apply_seccomp()` call at `:3516`. The document contradicted itself; the table was right. |
| A-16 | partial (4 of 6) | **RESOLVED per item** | (1) UVM handle-id offset — **FIXED**, both sides read the same profile field (`nvkvm_ioctl.c:330-341`, `nvkvm_main.c:1331-1376`, `:2305-2307`). (2) unbounded pinning — **OPEN by choice**, `grep -rn RLIMIT_MEMLOCK src/` is empty. (3) migration/mmap leaks — **FIXED with the one documented residual still present** (`nvkvm_mmap.c:917-981` — the GPU-VMA branch of `nvkvm_efault_resolve` re-issues `MMAP_ON_ISOLATE` at `:971-980` into a local `mmap_token` that is discarded on return; **citation corrected at `65aa69f`:** the bound is `nvkvm_iso_mmap_reap_handle()`, defined `nvkvm_isolate_handlers.c:4059` and called from the close-handle path at `:931`, not `:3683`). (4) handle across a lock drop — **CANNOT DETERMINE**, not re-traced; needs a ~65k-allocation repro, left as found. (5) transaction-id leak — **FIXED**, 14+ paired `nvkvm_txn_id_free()` sites. (6) `nvkvm_ring_has_work` — **FIXED**, both counters acquire-loaded (`nvkvm_ring.h:259-260`), ring pointer `READ_ONCE` (`nvkvm_main.c:339-345`). Net: **five of six done, one undetermined.** |
...
| A-18 | fixed | **PARTIAL — matches its own residual note** | **Citation corrected at `65aa69f`: these sites are in `nvkvm_isolate_handlers.c`, not `nvkvm_present_egl.c`** — that file has no `lseek`, no `buf_size` and no `nvkvm_present_geom_ok` at all. Helper `nvkvm_isolate_handlers.c:1501-1539`; pre-export `nvkvm_present_geom_ok(req, 0)` `:1579`; post-export `lseek(dmabuf_fd, 0, SEEK_END)` `:1629` re-checked at `:1637`. The residual holds: `if (buf_size && need > buf_size)` (`:1530`) short-circuits the size check when `lseek` returns ≤ 0, and both call sites pass 0 when the extent is unknown. |
...
| **P-4** | *see below* | **RESOLVED: PARTIAL** | `nvkvm_req_realize_uvm_mapping` — **FIXED** (`nvkvm_isolate_handlers.c:4496-4505`, in the handler at `:4472`; checks `fh->session_id == req->session_id` *and* `session_has_isolate`); `fd_handle_id` is now read (`:4496`), so the doc's *"grep returns nothing"* no longer holds. `nvkvm_req_munmap_on_isolate` — **STILL OPEN**, and the doc was right that it needs a protocol field: `struct nvkvm_req_munmap_on_isolate` is still `{isolate_id, mmap_token}` (`src/common/nvkvm_proto.h:484-487`). Partial mitigation landed (token must belong to the named isolate, `:4000`) with an explicit KNOWN GAP comment (`:3979`). |
