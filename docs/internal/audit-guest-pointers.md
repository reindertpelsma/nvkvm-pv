...
`iso_mmap_covers()` requires full containment in ranges the host installed.
**CORRECTED 2026-08-24:** this sentence originally read *"full containment in **one**
such entry, deliberately not a union (same reasoning as `uvm_va_covers()`)"*, which
describes a version of the function that was superseded before it shipped. The code
does a deliberate **union walk** over adjacent host-installed chunks
(`nvkvm_isolate_handlers.c:96-135`), because the migration path installs a
registration in 2 MiB pieces — one `mmap_on_isolate`, and therefore one table entry,
per chunk — so single-entry containment would have refused every `cudaHostRegister`
above 2 MiB. The function's own comment (`:59-80`) records this. The security
property is unchanged: full, unbroken coverage by `stub_mirrored` ranges only, with
any gap failing closed. Anything else is refused
...
- Verified unchanged and correct: U-1, U-2, U-3, U-4, U-5, U-6, U-10, U-12, U-15,
  A-1 (the gate itself), A-2. Verified still open: U-8, U-9, U-11, U-13.
...
(`b26c56f`, `ac87dfa`) and in code comments (`nvkvm_isolate_handlers.c:57`, `:2382`,
`:4101`).
