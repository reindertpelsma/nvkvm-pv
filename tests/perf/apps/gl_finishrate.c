/* gl_finishrate.c — isolate the cost of ONE GL sync (glDrawArrays + glFinish).
 *
 * glmark2's off-screen mode ends every frame with glFinish, so this is the
 * per-frame cost that dominates its score once the drawing itself is cheap.
 * Headless (EGL_EXT_platform_device), 16x16 FBO, one triangle -- the GPU work
 * is ~nothing, so what is measured is the submit + wait-for-idle round trip.
 *
 * argv[1] = iterations (default 20000).  Build:
 *   gcc -O2 gl_finishrate.c -o gl_finishrate -lEGL -lGLESv2
 */
...
int main(int argc,char**argv){
    ...
    for(int i=0;i<2000;i++){glDrawArrays(GL_TRIANGLES,0,3);glFinish();}
    double t0=now();
    for(int i=0;i<N;i++){glDrawArrays(GL_TRIANGLES,0,3);glFinish();}
    double dt=now()-t0;
    printf("METRIC finish_us %.2f\nMETRIC finish_per_s %.0f\nMETRIC finish_wall_s %.3f\n",
           dt/N*1e6, N/dt, dt);
    return 0;
}
